...
//-----------------------------------------------\\
	...
	var analyze = require(path.join(__dirname,"..","ml","test2.js"));
...
var target = process.argv[2];
	console.log(`pulldown on @${target}`);
...
//				.then(data => console.log("buildData returns:\n",JSON.stringify(data,null,"\t")))
			...
			//		self.tweet({"status": `@alicemazzy I am ${botornot.num}% sure @${target} is a ${botornot.what}`});
					console.log(`I am ${botornot.num}% sure @${target} is a ${botornot.what}`);
				});
//	}, 1000*10);
...
//	stream.on("delete", tweet => console.log("DELETED STATUS\n" + JSON.stringify(tweet) + "\nDELETE END"));
