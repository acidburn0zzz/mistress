	...
	pulldown: () => {
		...
		this.pulldown({screen_name: _.last(args)})
			.then(data => {
				...
				fs.writeFile(path.join(__dirname,"..","ml","training",_.last(args)+".json"),JSON.stringify(data,null,"\t"),"utf8",
					err=>{
						if(err) throw err;
						console.log(`saved to ${path.join(__dirname,"..","ml","training",_.last(args)+".json")}`);
					});
			});
	},
